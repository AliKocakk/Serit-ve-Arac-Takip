import cv2  # Computer Vision için.
import numpy as np  # Hesaplamalar için.
import edge_detection as edge  # Şerit tanıma için.
import matplotlib.pyplot as plt  # Grafik ve hata kontrolü için.
from ultralytics import YOLO # Şerit Tanıma için.
...
scale_ratio = 1  # Orijinal boyutun bir kesirine ölçekleme seçeneği.
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        self.YM_PER_PIX = 7.0 / 400  # meters per pixel in y
        self.XM_PER_PIX = 3.7 / 255  # meters per pixel in x
    ...
    def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
        ...
        if plot == True:
            ...
            # Adding color to the left and right line pixels
...
def main():
    ...
    result = cv2.VideoWriter('seritcikti.mp4', fourcc, 150.0, (int(cap.get(3)), int(cap.get(4))))
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            yolo_frame_shape = annotated_frame.shape
            frame_with_lane_lines2_resized = cv2.resize(frame_with_lane_lines2, (yolo_frame_shape[1], yolo_frame_shape[0]))

            combined_frame = cv2.addWeighted(frame_with_lane_lines2, 0.65, annotated_frame, 0.35, 0)
            result.write(combined_frame)
            cv2.imshow("Combined Frame", combined_frame)

            # Display the frame
           #combined_image = np.hstack((frame_with_lane_lines2, annotated_frame))
